...
export default function EquipmentPage() {
  ...
  const save = (values: EquipmentForm) => {
    const normalized = {
      ...values,
      kind,
      type: kind === "Mobil araç" ? "Mobil araç" : values.type.trim(),
    ...
    setEquipment((current) =>
      editor.item
        ...
        : [...current, { ...normalized, id: Date.now() }],
    );
  ...
  return (
    <Page>
      ...
      <div className="border-divider mt-5 flex gap-1 border-b" role="tablist" aria-label="Varlık türü">
        {(["Ekipman", "Mobil araç"] as EquipmentKind[]).map((item) => (
          <button
            aria-selected={kind === item}
            className={`relative flex items-center gap-2 px-4 py-3 text-sm font-semibold transition-colors ${kind === item ? "text-brand" : "text-muted hover:text-foreground"}`}
            key={item}
            onClick={() => {
              setKind(item);
              setStatus("Tümü");
          ...
          >
            ...
            {item}
            {kind === item && <span className="bg-brand absolute right-3 bottom-0 left-3 h-0.5 rounded-full" />}
          </button>
        ))}
      </div>
      <Card aria-label="Ekipman filtreleri" className="mt-7 p-4 sm:p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <Select
            aria-label="Ekipman durumu"
            className="h-10 w-full text-xs lg:w-52"
      ...
      {filtered.length === 0 ? (
        ...
        <Card className="mt-6 overflow-hidden p-0">
          <div className="hidden overflow-x-auto md:block">
            <table className="w-full text-left">
              ...
              <tbody className="divide-divider divide-y">
                {filtered.map((item) => (
                  <EquipmentRow item={item} key={item.id} onEdit={openEdit} onRemove={remove} />
                ))}
              </tbody>
            </table>
          </div>
          <div className="divide-divider divide-y md:hidden">
            {filtered.map((item) => (
              <EquipmentMobileCard item={item} key={item.id} onEdit={openEdit} onRemove={remove} />
            ))}
          </div>
        </Card>
      )}
