...
import { AlertTriangle, Check, Download, FileSpreadsheet, FileText, Pencil, Plus, Search, Trash2, Upload, UsersRound } from "lucide-react";
...
import { personnelShifts, personnelStatuses, nextPersonnelId, type Personnel, type PersonnelShift, type PersonnelStatus } from "@/lib/personnel";
...
export default function PersonnelPage() {
  ...
  const remove = (item: Personnel) => confirm({ title: "Personeli sil", description: `${item.name} personel kaydı kalıcı olarak silinecek.`, confirmLabel: "Personeli sil", onConfirm: () => { setPersonnel((current) => current.filter((entry) => entry.id !== item.id)); showNotice("Personel kaydı silindi."); } });
  ...
  return <Page>
    <PageHeader
      ...
      actions={<div className="flex flex-wrap gap-2"><input accept=".xlsx,.csv" className="hidden" onChange={(event) => void handleFiles(Array.from(event.target.files ?? []))} ref={excelFileRef} type="file" /><input accept=".pdf" className="hidden" multiple onChange={(event) => void handleFiles(Array.from(event.target.files ?? []))} ref={pdfFileRef} type="file" /><Button onClick={openImport} variant="outline"><Upload /> Toplu personel ekle</Button><Button onClick={() => void exportPersonnelExcel(filtered, companies, false)} variant="outline"><Download /> Excel&apos;e aktar</Button><Button onClick={() => void exportPersonnelExcel([], companies, true)} variant="secondary"><FileSpreadsheet /> Şablon indir</Button><Button onClick={openNew}><Plus /> Personel ekle</Button></div>}
    ...
    <Card className="mt-5 p-4 sm:p-5"><div className="flex flex-col gap-3 xl:flex-row xl:items-end"><Field className="min-w-0 flex-1" label="Personel ara"><span className="relative block"><Search className="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-subtle" /><Input aria-label="Personel ara" className="h-10 pl-9" onChange={(event) => { setQuery(event.target.value); resetPage(); }} placeholder="Ad, personel no, görev veya firma ara..." value={query} /></span></Field><Field label="Firma"><Select aria-label="Firma filtresi" className="h-10 min-w-52" onChange={(event) => { setCompanyFilter(event.target.value); resetPage(); }} value={companyFilter}><option value="Tümü">Tüm firmalar</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</Select></Field><Field label="Durum"><Select aria-label="Durum filtresi" className="h-10 min-w-36" onChange={(event) => { setStatusFilter(event.target.value); resetPage(); }} value={statusFilter}><option>Tümü</option>{personnelStatuses.map((status) => <option key={status}>{status}</option>)}</Select></Field><Field label="Departman"><Select aria-label="Departman filtresi" className="h-10 min-w-44" onChange={(event) => { setDepartmentFilter(event.target.value); resetPage(); }} value={departmentFilter}>{departments.map((department) => <option key={department}>{department}</option>)}</Select></Field></div><div className="mt-4 flex items-center justify-between text-xs text-muted"><span>Filtrelenen kayıtlar</span><CountPill>{filtered.length} personel</CountPill></div></Card>
    <DataTable className="mt-5" empty={filtered.length === 0 ? <EmptyState compact className="rounded-none border-0" description="Filtreleri değiştirin veya yeni personel ekleyin." icon={UsersRound} title="Personel bulunamadı" /> : undefined} mobile={paged.map((item) => <MobilePersonnel company={companies.find((company) => company.id === item.companyId)?.name ?? "Firma bulunamadı"} item={item} key={item.id} onDelete={() => remove(item)} onEdit={() => openEdit(item)} />)}><THead><Tr><Th>Personel</Th><Th>Firma</Th><Th>Görev / departman</Th><Th>İletişim</Th><Th>Durum</Th><Th>İşe giriş</Th><Th className="text-right">İşlem</Th></Tr></THead><TBody>{paged.map((item) => <Tr key={item.id}><Td><div className="flex items-center gap-3"><Avatar size="sm" text={initials(item.name)} /><div><Link className="text-xs font-semibold text-foreground hover:text-brand" href={`/personeller/${item.id}`}>{item.name}</Link><p className="text-[11px] text-muted">{item.employeeNo || "Personel no yok"}</p></div></div></Td><Td><span className="text-xs font-medium text-foreground">{companies.find((company) => company.id === item.companyId)?.name ?? "Firma bulunamadı"}</span></Td><Td><p className="text-xs font-semibold text-foreground">{item.title || "Görev belirtilmedi"}</p><p className="text-[11px] text-muted">{item.department || "Departman belirtilmedi"}</p></Td><Td><p className="text-[11px] text-muted">{item.email || "E-posta yok"}</p><p className="mt-1 text-[11px] text-muted">{item.phone || "Telefon yok"}</p></Td><Td><Badge tone={statusTone[item.status]}>{item.status}</Badge></Td><Td><span className="text-xs text-muted">{isoToLabel(item.startDate) || "Belirtilmedi"}</span></Td><Td><div className="flex justify-end gap-1"><Button aria-label={`${item.name} düzenle`} onClick={() => openEdit(item)} size="icon-sm" variant="ghost"><Pencil /></Button><Button aria-label={`${item.name} sil`} onClick={() => remove(item)} size="icon-sm" variant="danger"><Trash2 /></Button></div></Td></Tr>)}</TBody></DataTable>
...
function PersonnelModal({ companies, item, onClose, onSave, open }: { companies: ReturnType<typeof useCompanies>[0]; item: Personnel | null; onClose: () => void; onSave: (form: PersonnelForm) => void; open: boolean }) {
  ...
  return <Modal description="Personeli bir firmaya bağlayın ve temel operasyon bilgilerini kaydedin." footer={<><Button onClick={onClose} variant="outline">Vazgeç</Button><Button onClick={submit}><Check /> {item ? "Değişiklikleri kaydet" : "Personeli ekle"}</Button></>} icon={item ? Pencil : Plus} onClose={onClose} open={open} size="lg" title={item ? "Personeli düzenle" : "Firmaya personel ekle"}><div className="grid gap-4 sm:grid-cols-2"><Field error={submitted ? errors.companyId : ""} label="Firma" required><Select invalid={submitted && Boolean(errors.companyId)} onChange={(event) => setField("companyId", Number(event.target.value))} value={String(form.companyId)}><option value="0">Firma seçin</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</Select></Field><Field label="Sicil / personel no"><Input onChange={(event) => setField("employeeNo", event.target.value)} placeholder="Örn. ART-0024" value={form.employeeNo} /></Field><Field className="sm:col-span-2" error={submitted ? errors.name : ""} label="Ad soyad" required><Input autoFocus invalid={submitted && Boolean(errors.name)} onChange={(event) => setField("name", event.target.value)} placeholder="Örn. Ayşe Demir" value={form.name} /></Field><Field label="TC / yabancı kimlik no"><Input onChange={(event) => setField("nationalId", event.target.value)} placeholder="Yabancı personelde boş bırakılabilir" value={form.nationalId} /></Field><Field label="Doğum tarihi"><Input onChange={(event) => setField("birthDate", event.target.value)} type="date" value={form.birthDate} /></Field><Field label="Görev / unvan"><Input onChange={(event) => setField("title", event.target.value)} placeholder="Örn. Üretim operatörü" value={form.title} /></Field><Field label="Departman"><Input onChange={(event) => setField("department", event.target.value)} placeholder="Örn. Üretim" value={form.department} /></Field><Field label="E-posta"><Input onChange={(event) => setField("email", event.target.value)} type="email" value={form.email} /></Field><Field label="Telefon"><Input onChange={(event) => setField("phone", event.target.value)} type="tel" value={form.phone} /></Field><Field label="İşe giriş tarihi"><Input onChange={(event) => setField("startDate", event.target.value)} type="date" value={form.startDate} /></Field><Field label="Durum"><Select onChange={(event) => setField("status", event.target.value as PersonnelStatus)} value={form.status}>{personnelStatuses.map((status) => <option key={status}>{status}</option>)}</Select></Field><Field label="Vardiya"><Select onChange={(event) => setField("shift", event.target.value as PersonnelShift)} value={form.shift}>{personnelShifts.map((shift) => <option key={shift}>{shift}</option>)}</Select></Field><Field className="sm:col-span-2" label="Not"><Textarea onChange={(event) => setField("notes", event.target.value)} placeholder="Operasyonel not..." value={form.notes} /></Field></div></Modal>;
}
...
async function parsePersonnelFile(file: File, defaultCompanyId: number, defaultStartDate: string): Promise<ImportRow[]> {
  ...
  rows.slice(1).forEach((values, index) => {
    ...
    const rawShift = cellText(values[indexes.shift]);
    const status = personnelStatuses.find((item) => normalizedText(item) === normalizedText(rawStatus)) ?? "Aktif";
    const shift = personnelShifts.find((item) => normalizedText(item) === normalizedText(rawShift)) ?? "Belirtilmedi";
...
async function exportPersonnelExcel(items: Personnel[], companies: ReturnType<typeof useCompanies>[0], template: boolean) {
  ...
  const headers = ["Firma", "Ad Soyad", "TC No", "Sicil No", "Doğum Tarihi", "Görev", "Departman", "E-posta", "Telefon", "İşe Giriş Tarihi", "Durum", "Vardiya", "Not"];
  sheet.addRow(headers);
  if (!template) items.forEach((item) => sheet.addRow([companies.find((company) => company.id === item.companyId)?.name ?? "", item.name, item.nationalId, item.employeeNo, item.birthDate, item.title, item.department, item.email, item.phone, item.startDate, item.status, item.shift, item.notes]));
